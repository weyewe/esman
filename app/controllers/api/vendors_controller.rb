class Api::VendorsController < Api::BaseApiController
  
  def index
    
    if params[:livesearch].present? 
      livesearch = "%#{params[:livesearch]}%"
      @objects = Vendor.where{ 
        (
          (name =~  livesearch ) | 
          (address =~  livesearch ) |
          (description =~  livesearch ) 
           
        )
        
      }.page(params[:page]).per(params[:limit]).order("id DESC")
      
      @total = Vendor.where{
        (
          (name =~  livesearch ) | 
          (address =~  livesearch ) |
          (description =~  livesearch ) 
        )
        
      }.count
    else
      @objects = Vendor.active_objects.page(params[:page]).per(params[:limit]).order("id DESC")
      @total = Vendor.active_objects.count
    end
    
    
    
    # render :json => { :vendors => @objects , :total => @total, :success => true }
  end

  def create
    @object = Vendor.create_object( params[:vendor] )  
    
    
 
    if @object.errors.size == 0 
      render :json => { :success => true, 
                        :vendors => [@object] , 
                        :total => Vendor.active_objects.count }  
    else
      msg = {
        :success => false, 
        :message => {
          :errors => extjs_error_format( @object.errors )  
        }
      }
      
      render :json => msg                         
    end
  end

  def update
    
    @object = Vendor.find_by_id params[:id] 
    @object.update_object( params[:vendor])
     
    if @object.errors.size == 0 
      render :json => { :success => true,   
                        :vendors => [@object],
                        :total => Vendor.active_objects.count  } 
    else
      msg = {
        :success => false, 
        :message => {
          :errors => extjs_error_format( @object.errors )  
        }
      }
      
      render :json => msg 
    end
  end

  def destroy
    @object = Vendor.find(params[:id])
    @object.delete_object

    if @object.is_deleted
      render :json => { :success => true, :total => Vendor.active_objects.count }  
    else
      render :json => { :success => false, :total => Vendor.active_objects.count }  
    end
  end
  
  def search
    search_params = params[:query]
    selected_id = params[:selected_id]
    if params[:selected_id].nil?  or params[:selected_id].length == 0 
      selected_id = nil
    end
    
    query = "%#{search_params}%"
    # on PostGre SQL, it is ignoring lower case or upper case 
    
    if  selected_id.nil?
      @objects = Vendor.active_objects.where{
                                (
                                  (name =~  query ) | 
                                  (address =~  query ) | 
                                  (description =~  query ) 
                                )
                              }.
                        page(params[:page]).
                        per(params[:limit]).
                        order("id DESC")
                        
      @total = Vendor.active_objects.where{ 
                                (
                                  (name =~  query ) | 
                                  (address =~  query ) |
                                  (description =~  query ) 
                                )
                              }.count
    else
      @objects = Vendor.where{ (id.eq selected_id)  
                              }.
                        page(params[:page]).
                        per(params[:limit]).
                        order("id DESC")
   
      @total = Vendor.where{ (id.eq selected_id)   
                              }.count 
    end
    
    
    render :json => { :records => @objects , :total => @total, :success => true }
  end
end
